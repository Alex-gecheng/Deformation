...
private:
    ...
    vector<Eigen::Vector2d> constraint_initial_uv;
    vector<Eigen::Vector2d> constraint_active_uv;
    vector<Eigen::Vector2d> constraint_target_uv;
...
private:
    ...
    vector<Eigen::Matrix2d> local_J;

    // ========= 全局能量 =========
    double computeTotalEnergy() const;

    // ========= Anderson 加速 =========
    vector<Eigen::Matrix2Xd> Q_history;
    vector<Eigen::Matrix2Xd> G_history;
    vector<Eigen::Matrix2Xd> F_history;
    int anderson_history_m = 5;

    // ========= 优化 =========
    Eigen::Matrix2d project_to_valid_distortion(const Eigen::Matrix2d& J);
    void localStep();
    bool globalStep();
    Eigen::Matrix2Xd AndersonAcceleration(const Eigen::Matrix2Xd& UV_k,const Eigen::Matrix2Xd& G_k,const Eigen::Matrix2Xd& F_k);
